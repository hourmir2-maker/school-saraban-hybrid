...
export default function Orders() {
  ...
  async function handleDelete(id: string) {
    ...
    try {
      const { data: doc } = await supabase.from('orders').select('file_url').eq('id', id).maybeSingle();
      if (doc?.file_url) {
  ...
  const handleAttachFileSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const sanitized = (selectedDocForAttach.subject || 'คำสั่ง').replace(/[\/\\?%*:|"<>]/g, '-').slice(0, 50);
      const fileName = `คำสั่ง_${selectedDocForAttach.order_number}_เรื่อง_${sanitized}.${ext}`;
      const file_url = await uploadFileToDrive(attachFile, 'orders', fileName.replace('.' + ext, ''));
      ...
      alert('แนบไฟล์คำสั่งย้อนหลังเรียบร้อยแล้ว');
...
[พิมพ์หน้าที่รับผิดชอบข้อที่ 2 ของฝ่าย${g}]
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal isOpen={isAttachModalOpen} onClose={() => setIsAttachModalOpen(false)} title="📎 แนบไฟล์คำสั่งย้อนหลัง">
        <form onSubmit={handleAttachFileSubmit} className="space-y-4 text-slate-700">
          <div className="bg-amber-50/60 p-4 rounded-2xl border border-amber-200 space-y-2">
            ...
            <p className="text-sm font-bold text-slate-800">เลขที่คำสั่ง: {selectedDocForAttach?.order_number}</p>
