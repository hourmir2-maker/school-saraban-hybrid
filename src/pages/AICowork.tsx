...
export default function AICowork() {
  ...
  const handleAnalyzeDocument = async () => {
    ...
    try {
      ...
      let q = supabase.from('settings').select('gemini_api_key, ai_cowork_api_key, custom_sop');
    ...
    <div className="flex flex-col h-[calc(100vh-140px)] gap-6">
      ...
      <div className="flex items-center gap-2 bg-white p-1.5 rounded-2xl border border-slate-100 w-fit shadow-sm">
        ...
        <button 
          ...
          className={`flex items-center gap-2 px-6 py-2.5 rounded-xl font-black text-sm transition-all ${activeView === 'checklist' ? 'bg-purple-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}
        >
          <FileSearch size={18} /> AI Checklist & SOP
        </button>
      </div>
      {activeView === 'chat' && (
