...
ตอบกลับเฉพาะข้อความที่เป็นคำถามเดี่ยว (Standalone Query) ในภาษาไทยเท่านั้น ห้ามอธิบายเพิ่มเติม ห้ามมีเครื่องหมายคำพูด ห้ามขึ้นต้นด้วย "คำถามเดี่ยว:"`;
  ...
  // กรองคำบอกวันที่ออกก่อนส่งให้ AI Planner เพื่อป้องกัน AI สร้าง filter วันที่ผิดพลาด
  ...
  if (!hasDocQuery && isDocRelated) {
    ...
    const parseDateRange = (text: string): { gte?: string; lte?: string } | null => {
      ...
      // --- กรองวันที่แบบเฉพาะเจาะจง เช่น "วันที่ 15" "1 กรกฎาคม" "15/07/2568" ---
      const THAI_MONTHS: Record<string, number> = {
        'มกราคม': 1, 'กุมภาพันธ์': 2, 'มีนาคม': 3, 'เมษายน': 4,
        'พฤษภาคม': 5, 'มิถุนายน': 6, 'กรกฎาคม': 7, 'สิงหาคม': 8,
        'กันยายน': 9, 'ตุลาคม': 10, 'พฤศจิกายน': 11, 'ธันวาคม': 12
      };
      // รูปแบบ: "15 กรกฎาคม 2568" หรือ "15 กรกฎาคม"
      ...
      // รูปแบบ: "วันที่ 15" (ไม่ระบุเดือน = เดือนนี้)
      ...
      return null; // ไม่ได้ระบุวันที่ = ดึงทั้งหมด
    };
    ...
    // หากไม่ระบุประเภท ให้ดึงทั้ง incoming และ outgoing
